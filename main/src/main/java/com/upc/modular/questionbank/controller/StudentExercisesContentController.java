...
public class StudentExercisesContentController {
    ...
    @ApiOperation("删除学生做题内容")
    @PostMapping("deleteStudentExercisesContent")
    public R deleteStudentExercisesContent(@RequestBody IdParam idParam){
        studentExercisesContentService.deleteStudentExercisesContentByIds(idParam);
        return R.commonReturn(200, "删除成功", "");
    }

    @ApiOperation("更新学生做题内容")
    @PostMapping("updateStudentExercisesContent")
    public R updateStudentExercisesContent(@RequestBody StudentExercisesContent param){
        studentExercisesContentService.updateStudentExercisesContent(param);
        return R.commonReturn(200, "修改成功", "");
    }

    @ApiOperation("根据id查询单个学生做题内容")
    @PostMapping("selectStudentExercisesContent")
    public R<StudentExercisesContent> selectStudentExercisesContent(@RequestBody StudentExercisesContent param){
        StudentExercisesContent result = studentExercisesContentService.getById(param);
        return R.ok(result);
    }

    @ApiOperation("分页查询学生做题内容")
    @PostMapping("selectStudentExercisesContentPage")
    public R<PageBaseReturnParam<StudentExercisesContent>> selectStudentExercisesContentPage(@RequestBody StudentExercisesContentPageSearchParam param){
        Page<StudentExercisesContent> page = studentExercisesContentService.selectStudentExercisesContentPage(param);
        PageBaseReturnParam<StudentExercisesContent> p = PageBaseReturnParam.ok(page);
        return R.page(p);
    }
